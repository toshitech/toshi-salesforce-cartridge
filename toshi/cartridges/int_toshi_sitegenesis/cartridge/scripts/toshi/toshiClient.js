...
exports.checkAddressEligible = function(address) {
    var eligible = false;
    try {
        var country = address.countryCode ? address.countryCode.toLowerCase() : '';
        ...
        if (toshiConfig) {
            ...
            if (country in toshiKey) {
                ...
                var result = toshiService.call({ request : requestParams, apiKey: apiKey});
...
function sendOrder(order){
    ...
    if (toshiConfig) {
        var toshiKey = JSON.parse(toshiConfig);
        if (shippingCountry in toshiKey) {
            let apiKey = toshiKey[shippingCountry]['server'];
            var toshiService = toshiServiceClient('toshi.api.createorder');
            var result = toshiService.call({ request : requestObject, apiKey: apiKey});
            if (result.status != 'OK' && !result.ok) {
                sendNotification(result);
            }
...
function sendNotification(svResult){
    ...
    let notificationUser = currentSite.getCustomPreferenceValue('toshiNotificationList');
